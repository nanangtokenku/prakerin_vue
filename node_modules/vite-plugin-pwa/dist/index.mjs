...
function createAPI(ctx) {
  return {
    ...
    webManifestData() {
      ...
      if (!options2 || options2.disable || ctx.viteConfig.build.ssr || ctx.devEnvironment && !ctx.options.devOptions.enabled)
        return void 0;
    ...
    load(id) {
      ...
      if (VIRTUAL_MODULES.includes(id)) {
        ...
        if (ctx.viteConfig.command === "serve" && ctx.options.devOptions.enabled) {
          return generateRegisterSW2(__spreadProps(__spreadValues({}, ctx.options), { filename: swDevOptions.swUrl }), "build", VIRTUAL_MODULES_MAP[id]);
        } else {
          return generateRegisterSW2(ctx.options, !ctx.options.disable && ctx.viteConfig.command === "build" ? "build" : "dev", VIRTUAL_MODULES_MAP[id]);
        }
      }
    },
    api
